...
exports.run = async (client, message, args) => {
    ...
    if (!args[0])
        return message.channel.send("Opções: `pedra`, `papel` ou `tesoura`").then(msg => msg.delete({ timeout: 10000 }))

    const Options = ["pedra", "papel", "tesoura"]
    if (!Options.includes(args[0]))
        return message.channel.send(":x: Opção Incorreta!").then(m => m.delete({ timeout: 5000 }))
    ...
    if (args[0] === "pedra") {
        ...
        message.channel.send(random1[Math.floor(Math.random() * random1.length)]).then(m => m.delete({ timeout: 5000 }))
        ...
        message.channel.send(random2[Math.floor(Math.random() * random2.length)]).then(m => m.delete({ timeout: 5000 }))
        ...
        message.channel.send(random3[Math.floor(Math.random() * random3.length)]).then(m => m.delete({ timeout: 5000 }))
    }
}