...
module.exports.run = async (bot, message, args) => {
    ...
    if (!perms) {
        ...
        return message.channel.send(permss).then(msg => msg.delete({ timeout: 3000 }))
    }
    ...
    if (!user) {
        ...
        return message.channel.send(userr).then(msg => msg.delete({ timeout: 3000 }))
    }
    ...
    if (!role) {
        const rolee = new Discord.MessageEmbed()
            ...
            .setTitle('Siga o formato')
            .setDescription('`-addrole @user @cargo`')
        return message.channel.send(rolee).then(msg => msg.delete({ timeout: 3000 }))
    }
    ...
    if (user.roles.cache.has(role.id)) {
        ...
        return message.channel.send(roleee).then(msg => msg.delete({ timeout: 3000 }))
    }
    ...
    message.channel.send(sucess).then(msg => msg.delete({ timeout: 6000 }))
}