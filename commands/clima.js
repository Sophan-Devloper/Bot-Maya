...
exports.run = async (client, message, args) => {
  message.delete()
    ...
    await weather.find({search: city, degreeType: degreetype}, function(err, result) {
        ...
        if (!city)
          return message.channel.send("Ei, espera. Qual é a cidade?\nTenta assim `-clima NomeDaCidade` etc").then(msg => msg.delete({timeout: 6000}))

        if (err || result === undefined || result.length === 0)
         return message.channel.send("Eu não conheço essa cidade, acho que você escreveu errado.").then(msg => msg.delete({timeout: 6000}))
