...
exports.run = async (client, message, args) => {
  ...
  if (!canal)
    return message.channel.send("Por favor, peça para que um Adm crie um Canal com o nome `sugestões` \nCopie e cole isso que eu crio um pra você \n`-createchannel sugestões`").then(msg => msg.delete({ timeout: 10000 }))

  if (!args[0])
    return message.reply('escreva a sua ideia assim -> `-ideia Suas ideias`').then(msg => msg.delete({ timeout: 5000 }))

  else if (content.length > 1000) {
    return message.channel.send(`${message.author.username}, forneça uma sugestão de no máximo 1000 caracteres.`).then(msg => msg.delete({ timeout: 5000 }))
