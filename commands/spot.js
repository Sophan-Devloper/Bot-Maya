...
exports.run = async (client, message, args) => {
    ...
    if (status !== null && status.type === "LISTENING" && status.name === "Spotify" && status.assets !== null) {
        ...
        return message.channel.send(embed).then(msg => msg.delete({timeout: 8000})).then(msg => message.channel.send('Estou te enviando esta mesma mensagem no seu privado, vai que você queira ouvir está música depois.')).then(msg => message.author.send(embed)).then(msg => message.author.send("Prontinho, vou deixar a música aqui :hearts:"))
    }}
