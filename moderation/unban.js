...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(!perms) 
          return message.channel.send('Calminha aí, você não pode usar este comando!').then(msg => msg.delete({timeout: 6000}))

        let member = args[0];
        if (!member) 
          return message.channel.send('Por favor, me diga o ID do usúario que você quer desbanir. \n`Configurações do Servidor - Banimentos - Copie o ID do usúario`').then(msg => msg.delete({timeout: 8000}))
