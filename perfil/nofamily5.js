...
exports.run = async (client, message, args) => {
    ...
    if (!args[0]) {
        return message.channel.send('Marque o seu familiar `' + prefix + 'nofamily5 @Familiar`')
    }

    var par = user.id === db.get(`family5_${message.author.id}`)
    if (!par) {
        return message.channel.send(`${user} não é a pessoa que você está em um relacionamento.`)
    }

    await db.delete(`family5_${par}`)
