...
run: async (bot, message, args) => {
  message.delete()
    ...
    if (!message.member.permissions.has("MANAGE_CHANNELS"))
       return message.reply("Ooops, apenas a Staff do Servidor pode fazer sorteios.").then(msg => msg.delete({timeout: 5000}))
    
    if (!args[0])
       return message.channel.send('Comando Sorteio `-sorteio 1s/m/h/d #CanalDoSorteio Prêmio`\n\nSegundos/Minutos/Horas/Dias').then(msg => msg.delete({timeout: 15000}))
    ...
    if (isNaN(args[0][0]))
        return message.channel.send('Isso não é um número! Digite `-sorteio`, por favor.').then(msg => msg.delete({timeout: 15000}))
    let time = isNaN(args[0][0])

    let channel = message.mentions.channels.first();
    if (!channel)
      return message.channel.send('Eu não achei o canal do sorteio. Por favor, me diga.').then(msg => msg.delete({timeout: 15000}))
    ...
    m.react("🎉").then(msg => channel.send(`Tempo do Sorteio: ${args[0]}`)).then(msg => message.channel.send(`Eu criei o sorteio no canal: ${channel}, assim como foi pedido.`)).then(msg => msg.delete({timeout: 5000}))
